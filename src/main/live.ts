...
function handleRemux(target: string, back: number, res: http.ServerResponse): void {
  ...
  const proc = spawn(
    ...
    [
      ...
      '-c:v',
      'copy',
      // Görüntü değişmeden kopyalanırken ses yeniden kodlandığı için ikisi
      // arasında ufak bir kayma oluşup (ses önce gelip sonra "oturuyormuş"
      // gibi görünüyordu) düzelmesi biraz sürebiliyordu. aresample'ın async
      // düzeltmesi sesi kendi zaman damgasına göre hafifçe esnetip görüntüyle
      // hizalıyor.
      '-af',
